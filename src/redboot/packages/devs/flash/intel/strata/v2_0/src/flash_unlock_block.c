...
flash_unlock_block(volatile flash_t *block, int block_size, int blocks)
{
    ...
    ROM = CYGNUM_FLASH_BASE_MASK & (unsigned int)block;
    block = block;
    ...
    bp = ROM;
    for (i = 0;  i < blocks;  i++) {
        *FLASH_P2V(bp) = FLASH_Read_Query;
        if (bp == block) {
            is_locked[i] = 0;
        } else {
#if 8 == CYGNUM_FLASH_WIDTH
            is_locked[i] = FLASH_P2V(bp)[4];
#else
            is_locked[i] = FLASH_P2V(bp)[2];
# endif
        }
    ...
    FLASH_P2V(ROM)[0] = FLASH_Clear_Locks;
    FLASH_P2V(ROM)[0] = FLASH_Clear_Locks_Confirm;  // Confirmation
    timeout = 5000000;
    while(((stat = FLASH_P2V(ROM)[0]) & FLASH_Status_Ready) !=
          FLASH_Status_Ready) {
        if (--timeout == 0) break;
