...
int flash_erase_block(volatile flash_t *block, unsigned int block_size)
{
    ...
    ROM = (flash_t *)(CYGNUM_FLASH_BASE_MASK & (unsigned int)block);
    eb = block;
...
// FIXME - Handle 'boot' blocks
    ...
    // Erase block
    while (block_len > 0) {
        FLASH_P2V(ROM)[0] = FLASH_Block_Erase;
        *FLASH_P2V(eb) = FLASH_Confirm;
        timeout = 5000000;
        ...
        eb = (flash_t *)((unsigned int)eb + erase_block_size);
    }
    ...
    if (stat & FLASH_ErrorMask ) {
        len = block_size;
        while (len > 0) {
            if (*FLASH_P2V(block++) != FLASH_BlankValue ) break;
            len -= sizeof(*block);
        }
        if (len == 0) stat = 0;
    }
