...
//======================================================================================================================
// Class for managing the user programs configuration.
//
// Dependencies:
...
if (!window['$'])
   {
   var noJQuery = "The jQuery library is required by edu.cmu.ri.terk.UserProgramsManager.js";
   alert(noJQuery);
...
if (!JSON)
   {
   var msg3 = "The json2.js library is required by edu.cmu.ri.terk.UserProgramsManager.js";
   alert(msg3);
...
(function()
   ...
   edu.cmu.ri.terk.UserProgramsConfigManager = function(host)
      {
      ...
      var loadConfig = function(onSuccessCallbackFunction)
         {
         // load the config
         jQuery.ajax(
         {
            url: host + '/cgi-bin/getUserProgramsConfigAsJSON.pl',
      ...
      this.saveConfig = function()
         ...
         if (this.isModified())
            ...
            jQuery.ajax(
            {
               url: host + '/cgi-bin/saveUserProgramsConfig.pl',
               type: 'POST',
      ...
      this.getProgramToRunOnStartup = function()
         {
         return getJSONProperty(json, "user-programs.program-to-run-on-bootup");
         };

      this.setProgramToRunOnStartup = function(programName)
         {
         json['user-programs']['program-to-run-on-bootup'] = programName;
         notifyChangeListeners();
         };

      this.programNamesIterator = function(callbackFunction)
         {
         if (callbackFunction)
            {
            var programs = getJSONProperty(json, "user-programs.programs");
            jQuery.each(programs, function(i, programName)
               {
               callbackFunction(programName);
               });
            }
         };
